...
L.PM.Edit.Curve = L.PM.Edit.extend({
  includes: [CurveCommon, L.PM.Mixins.MarkerLimits],
  options: {
    lineGuideStyle: {
      color: '#ff3c3c',
      weight: 2,
  ...
  _initMarkers() {
    const coords = this._layer._coords;
  ...
  _setupControlMarkerEdit(markerIndex) {
    ...
    const nextIsCubic =
      markerIndex < nbInstructions - 1 &&
      this._instructions[markerIndex + 1] === 'C';
    ...
    if (currentInstruction === 'C') {
      if (nextIsCubic && !destSameSecondControl) {
        ...
        if (!isLast) {
          marker._coordsIndex.push([markerIndex, 1]);
        }
      ...
      if (nextIsCubic) {
        marker._coordsIndex.push([markerIndex + 1, 0]);
      }
    }
  ...
  _setupSimpleHandle(e) {
    ...
    const markerControl = this._createMarker(currentCoords[1], true).addTo(
      this._editLayers
    );
    const line = L.polyline(
      [currentCoords[1], currentCoords[2]],
      this.options.lineGuideStyle
    );
  ...
  _setupDoubleHandle(e) {
    ...
    const markerControl2 = this._createMarker(currentCoords[1], true).addTo(
      this._editLayers
    );
    const markerControl1 = this._createMarker(nextCoords[0], true).addTo(
      this._editLayers
    );
    this._initMarkerArgs(
      markerControl1,
      markerIndex,
      [[markerIndex + 1, 0]],
      { ref: [markerIndex, 2], marker: markerControl2 },
      lineArgs
    );
    this._initMarkerArgs(
      markerControl2,
      markerIndex,
      [[markerIndex, 1]],
      { ref: [markerIndex, 2], marker: markerControl1 },
      lineArgs
    );
    marker._line = lineArgs;
  ...
  _updateSymetric(latlng, symetricArgs) {
    ...
    const symetric = L.PM.Curve.Utils.getPointSymetric(pointRef, latlng);
  ...
  _onDragMarker(e) {
    ...
    if (indexMarker !== this.markerSelected) {
      ...
      if (e.target._setupDouble) {
        this._setupDoubleHandle(e);
      }
      if (e.target._setupSimple) {
        this._setupSimpleHandle(e);
      }
  ...
  _handleDrag(deltaLatLng) {
    ...
    const newCoords = L.PM.Utils.moveCoordsByDelta(
      deltaLatLng,
      L.PM.Curve.Utils.pathToCoordsOnly(this._layer)
    );
    // apply new coordinates
    L.PM.Curve.Utils.reconciliateCoords(this._layer, newCoords);
  },
  _handleRotate(latlngs) {
    L.PM.Curve.Utils.reconciliateCoords(this._layer, latlngs);
  },
});
